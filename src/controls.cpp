...
FontSelector::FontSelector(HWND hwnd, const MyFont &font, std::function<void(const MyFont &)> callback)
{
    CHOOSEFONTW cf;
    ...
    if (ChooseFontW(&cf))
    {
        MyFont f{lf.lfFaceName, cf.iPointSize / 10.0f, !!(cf.nFontType & BOLD_FONTTYPE), !!(cf.nFontType & ITALIC_FONTTYPE)};
        callback(f);
    }
}